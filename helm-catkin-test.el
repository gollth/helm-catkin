...
(ert-deftest test-helm-catkin-workspace-returns-path ()
  "Tests if the helm-catkin-workspace function returns a string to the current workspace"
  (with-mock
...
(ert-deftest test-helm-catkin--util-format-list-returns-string ()
  "Tests if the helm-catkin--util-format-list function returns a string for given inputs"
...
(ert-deftest test-helm-catkin--config-args-raises-if-env-not-set ()
  ...
  (with-mock
    ...
    (should-error (helm-catkin--config-args ""))      ;; no operation just
    )
  )
...
(ert-deftest test-helm-catkin-config-cmake-args-are-correct ()
  ...
  (with-mock
    (mock (getenv helm-catkin--WS) => path)
    (should (equal (helm-catkin-config-cmake-args) '("-DCMAKE_BUILD_TYPE=Release" "-DCMAKE_EXPORT_COMPILE_COMMANDS=ON")))
    )
  )
